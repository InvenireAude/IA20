...
struct Locale{
	...
	Locale();
	 ...
	 std::unordered_map<std::string,int>  hmMonths;
	 std::unordered_map<std::string,int>  hmDays;

  long int    iGMTOffset;
  const char *sTimeZone;

  const std::string CDftDateTimeFormatIn;
  const std::string CDftDateTimeFormatOut;
  const std::string CDftTimeFormat;
  const std::string CDftDateFormat;
};
...
/*************************************************************************/
Timestamp::Timestamp(const std::string& strValue) {
		fromDateTimestd::string(strValue);
}
/*************************************************************************/
...
/*************************************************************************/
std::string Timestamp::tostd::string() const {
	IA20_TRACER;
	return toDateTimestd::string();
}
/*************************************************************************/
std::string Timestamp::toDateTimestd::string() const {
	IA20_TRACER;
	return tostd::string(TheLocale.CDftDateTimeFormatOut);
}
/*************************************************************************/
std::string Timestamp::toTimestd::string() const {
	IA20_TRACER;
	return tostd::string(TheLocale.CDftTimeFormat);
}
/*************************************************************************/
std::string Timestamp::toDatestd::string() const {
	IA20_TRACER;
	return tostd::string(TheLocale.CDftDateFormat);
}
/*************************************************************************/
std::string Timestamp::tostd::string(const std::string& strFormat) const {
	IA20_TRACER;
  ...
  std::string::const_iterator it=strFormat.end();
  ...
  if( (*--it) == 'Z' && (*it) != '%'){
	...
	for(std::string::const_iterator it=strFormat.begin();
			it != strFormat.end();
			it++) {
        ...
        case 'z': ssValue<<(TheLocale.iGMTOffset < 0 ? "-" : "+")<<std::setw(2)<<abs(TheLocale.iGMTOffset/60)
				...
				default:
				IA20_THROW(BadUsageException(std::string("Invalid timestamp format:")+strFormat));
			}
		}
...
/*************************************************************************/
void Timestamp::fromstd::string(const std::string& strValue) {
	IA20_TRACER;
	fromDateTimestd::string(strValue);
}
/*************************************************************************/
void Timestamp::fromDateTimestd::string(const std::string& strValue) {
	IA20_TRACER;
	fromstd::string(strValue,TheLocale.CDftDateTimeFormatIn);
}
/*************************************************************************/
void Timestamp::fromDatestd::string(const std::string& strValue) {
	IA20_TRACER;
	fromstd::string(strValue,TheLocale.CDftDateFormat);
}
/*************************************************************************/
void Timestamp::fromTimestd::string(const std::string& strValue) {
	IA20_TRACER;
	fromstd::string(strValue,TheLocale.CDftTimeFormat);
}
/*************************************************************************/
void Timestamp::fromstd::string(const std::string& strValue, const std::string& strFormat) {
	IA20_TRACER;
	fromstd::string(strValue,strFormat,false);
}
/*************************************************************************/
void Timestamp::fromstd::string(const std::string& strValue, const std::string& strFormat, bool toLocalTime) {
	IA20_TRACER;
	...
	for(std::string::const_iterator it=strFormat.begin();
		...
		if(!bReplacement) {
			...
			if(*it == '%') {
				...
				IA20_THROW(BadUsageException(std::string("Error in timestamp std::string(1):")+strFormat+"/"+strValue));
			}
			...
			try {
				switch(*it) {
					...
					case '%': if(ssValue.get() != '%') IA20_THROW(BadUsageException(std::string("Error in timestamp std::string(2):")+strFormat+"/"+strValue)); break;
          ...
          case 'x': iZoneOff=readZoneOffset(ssValue) - TheLocale.iGMTOffset; break;
            ...
            break;
				...
				IA20_THROW(BadUsageException(std::string("Timestamp parse error(3):")+strFormat+"/"+strValue+e.getInfo()));
			}
...
short Timestamp::readUSec(std::istream& is, bool bOptionalDecimal) {
	...
	if(c != '.') {
		...
		if(bOptionalDecimal) {
			...
			IA20_THROW(BadUsageException("Parse error in a timestamp std::string, decimal point expected."));
		}
	}
	...
	if(iDigits == 0)
		IA20_THROW(BadUsageException("Parse error in a timestamp std::string, empty fraction."));
  ...
  if(!is.good()){
	...
	switch(c) {
		...
		default:
			...
			IA20_THROW(BadUsageException("Parse error [1] in a timestamp std::string, \'+/-/Z\' in zone expected:"));
	}
...
short Timestamp::readAbbreviatedMonth(std::istream& is) {
	...
	if(TheLocale.hmMonths.count(strValue) == 0)
		IA20_THROW(BadUsageException("Parse error in a timestamp std::string, uknown month abbreviated."));
